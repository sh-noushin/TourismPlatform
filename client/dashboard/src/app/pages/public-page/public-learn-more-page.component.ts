...
import { ApiException, Client, ProblemDetails, PublicLearnMorePageDto } from '../../api/client';
import { PublicLearnMoreEditComponent } from './public-learn-more-edit.component';

@Component({
  standalone: true,
  selector: 'public-learn-more-page',
...
export class PublicLearnMorePageComponent implements OnDestroy {
  ...
  readonly item = signal<PublicLearnMorePageDto | null>(null);
  ...
  readonly columns = computed<SfTableColumn[]>(() => {
    this.lang();
    return [
      { key: 'locale', header: 'Locale', field: 'locale', align: 'start' },
      { key: 'title', header: 'Title', field: 'title' },
      { key: 'heading', header: 'Heading', field: 'heading' },
  ...
  async load(force = false): Promise<void> {
    ...
    try {
      const data = await firstValueFrom(this.client.learnMoreGET(this.lang()));
      ...
      if (status === 404) {
        ...
        this.error.set(err?.message ?? 'Failed loading learn more page');
      }
    } finally {
  ...
  openCreateOrEdit(): void {
    const ref = this.dialog.open(PublicLearnMoreEditComponent, {
  ...
  onRowAction(event: { action: any; row: PublicLearnMorePageDto }) {
